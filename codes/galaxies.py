...
def get_ra_dec_z_region(galaxy_file: str) -> tuple:
    ...
    """
    Loads sky coordinate data for DESI galaxies.

    Parameters
    ----------
    galaxy_file : str
        Path to a .BIN file containing galaxies as a 1D array:
        [target_IDs, RA, Dec, redshift]

    Returns
    -------
    ra : np.ndarray
        R.A. coordinates in degrees, shape (N,)
    dec : np.ndarray
        Declination coordinates in degrees, shape (N,)
    redshift : np.ndarray
